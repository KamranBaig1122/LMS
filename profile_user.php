...
if(isset($_POST['save_button']))
{
		...
		if(in_array($img_ext, $extensions))
		{
			if($image_size <= 2000000)
			{
				if($width == 225 && $height == 225)
				{
					...
					if(move_uploaded_file($tmp_name, "pics/" . $new_img_name))
					{
	...
	if($message == '')
	{
		...
		$query = "UPDATE lms_user SET user_name = :user_name, user_address = :user_address, user_contact_no = :user_contact_no, 
            user_profile = :user_profile, user_email_address = :user_email_address, user_password = :user_password, 
            user_updated_on = :user_updated_on WHERE user_unique_id = :user_unique_id";
...
$query = "SELECT * FROM lms_user WHERE user_unique_id = '".$_SESSION['user_id']."'";
			...
			?>
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="mb-3">
						...
						<img src="pics/<?php echo $row['user_profile']; ?>" width="100" class="img-thumbnail" />
					</div>
