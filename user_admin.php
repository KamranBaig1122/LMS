...
if(isset($_GET["action"], $_GET['status'], $_GET['code']) && $_GET["action"] == 'delete')
{
	...
	$query = "UPDATE lms_user SET user_status = :user_status, 
    user_updated_on = :user_updated_on WHERE user_id = :user_id";

	$statement = $connect->prepare($query);

	$statement->execute($data);

	header('location:user_admin.php?msg='.strtolower($status).'');
}
    		...
    		<table id="datatablesSimple">
    			...
    			if($statement->rowCount() > 0)
    			{
    				foreach($statement->fetchAll() as $row)
    				{
    					...
    					echo '
    					<tr>
    						<td><img src="pics/'.$row["user_profile"].'" class="img-thumbnail" width="75" /></td>
...
<script>
	...
	function delete_data(code, status)
	{
		...
		{
			window.location.href = "user_admin.php?action=delete&code="+code+"&status="+new_status+"";
		}
	}
