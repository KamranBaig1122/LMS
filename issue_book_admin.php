...
if(isset($_POST["issue_book_button"]))
{
    ...
    if($error == '')
    {
        ...
        $query = "SELECT * FROM lms_book WHERE book_isbn_number = '".$formdata['book_id']."'";
        ...
        if($statement->rowCount() > 0)
        {
            foreach($statement->fetchAll() as $book_row)
            {
                ...
                if($book_row['book_status'] == 'Enable' && $book_row['book_no_of_copy'] > 0)
                {
                    ...
                    $query = "SELECT user_id, user_status FROM lms_user WHERE user_unique_id = '".$formdata['user_id']."'";
                        ...
                        foreach($statement->fetchAll() as $user_row)
                        {
                            if($user_row['user_status'] == 'Enable')
                            {
                                ...
                                if($total_book_issue < $book_issue_limit)
                                {
                                    ...
                                    header('location:issue_book_admin.php?msg=add');
                                }
                                else
                                {
...
if(isset($_POST["book_return_button"]))
{
    if(isset($_POST["book_return_confirmation"]))
    {
        ...
        $query = "UPDATE lms_issue_book SET return_date_time = :return_date_time, book_issue_status = :book_issue_status WHERE issue_book_id = :issue_book_id";

        $statement = $connect->prepare($query);

        $statement->execute($data);

        $query = "UPDATE lms_book SET book_no_of_copy = book_no_of_copy + 1 WHERE book_isbn_number = '".$_POST["book_isbn_number"]."'";

        $connect->query($query);

        header("location:issue_book_admin.php?msg=return");
    }
    else
    {
...
$query = "SELECT * FROM lms_issue_book ORDER BY issue_book_id DESC";
$statement = $connect->prepare($query);
    ...
    <?php 
        ...
        else if($_GET["action"] == 'view')
        {
            ...
            if($issue_book_id > 0)
            {
                ...
                {
                    $query = "SELECT * FROM lms_book WHERE book_isbn_number = '".$row["book_id"]."'";

                    $book_result = $connect->query($query);

                    $query = "SELECT * FROM lms_user WHERE user_unique_id = '".$row["user_id"]."'";
                    ...
                    <ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
                        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="issue_book_admin.php">Issue Book Management</a></li>
                    ...
                    foreach($user_result as $user_data)
                    {
                        ...
                        <table class="table table-bordered">
                            ...
                            <tr>
                                ...
                                <td width="70%"><img src="pics/' . $user_data["user_profile"].'" class="img-thumbnail" width="100" /></td>
                            </tr>
                        </table>
                        <br />
                        ';
    	...
    	<div class="card-header">
    		<div class="row">
                ...
                <div class="col col-md-6" align="right">
                    <a href="issue_book_admin.php?action=add" class="btn btn-success btn-sm">Add</a>
                </div>
            </div>
        </div>
        <div class="card-body">
        			...
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				if($row["book_issue_status"] == "Issue")
        				{
        					...
        					if($current_date_time > $expected_return_date)
        					{
        						...
        						$query = "UPDATE lms_issue_book SET book_fines = '".$book_fines."', 
								book_issue_status = '".$status."' WHERE issue_book_id = '".$row["issue_book_id"]."'";
        				...
        				<tr>
        					...
        					<td>'.$status.'</td>
        					<td>
                                <a href="issue_book_admin.php?action=view&code='.convert_data($row["issue_book_id"]).'" class="btn btn-info btn-sm">View</a>
                            </td>
        				</tr>
        				';
        			}
