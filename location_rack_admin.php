...
if(isset($_POST["add_location_rack"]))
{
	...
	if($error == '')
	{
		$query = "SELECT * FROM lms_location_rack WHERE location_rack_name = '".$formdata['location_rack_name']."'";
		...
		else
		{
			...
			header('location:location_rack_admin.php?msg=add');
		}
	}
}
...
if(isset($_POST["edit_location_rack"]))
{
	...
	if($error == '')
	{
		...
		$query = "SELECT * FROM lms_location_rack WHERE location_rack_name = '".$formdata['location_rack_name']."'AND location_rack_id != '".$location_rack_id."'";
		...
		else
		{
			...
			$query = "UPDATE lms_location_rack SET location_rack_name = :location_rack_name, location_rack_updated_on = :location_rack_updated_on  
			...
			header('location:location_rack_admin.php?msg=edit');
		}
	}
}
...
if(isset($_GET["action"], $_GET["code"], $_GET["status"]) && $_GET["action"]=='delete')
{
	...
	$query = "UPDATE lms_location_rack SET location_rack_status = :location_rack_status, location_rack_updated_on = :location_rack_updated_on 
    WHERE location_rack_id = :location_rack_id";
	...
	header('location:location_rack_admin.php?msg='.strtolower($status).'');
...
<div class="container-fluid py-4" style="min-height: 700px;">
	...
	<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
		...
		<li class="breadcrumb-item"><a href="location_rack_admin.php">Location Rack Management</a></li>
		...
		<div class="card-header">
			<div class="row">
				...
				<div class="col col-md-6" align="right">
					<a href="location_rack_admin.php?action=add" class="btn btn-success btn-sm">Add</a>
				</div>
			</div>
		</div>
		<div class="card-body">
				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						<tr>
							...
							<td>
								<a href="location_rack_admin.php?action=edit&code='.convert_data($row["location_rack_id"]).'" class="btn btn-sm btn-primary">Edit</a>
	...
	<script>
		...
		function delete_data(code, status)
		{
			...
			{
				window.location.href = "location_rack_admin.php?action=delete&code="+code+"&status="+new_status+""
			}
		}
