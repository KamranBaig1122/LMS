...
if(isset($_POST["add_book"]))
{
	...
	if($error == '')
	{
		...
		$query = "INSERT INTO lms_book (book_category, book_author, book_location_rack, book_name, book_isbn_number, book_no_of_copy, book_status, book_added_on) 
		...
		header('location:book_admin.php?msg=add');
	}
}
...
if(isset($_POST["edit_book"]))
{
	...
	if($error == '')
	{
		...
		$query = "UPDATE lms_book SET book_category = :book_category, book_author = :book_author, book_location_rack = :book_location_rack, 
        book_name = :book_name, book_isbn_number = :book_isbn_number, book_no_of_copy = :book_no_of_copy, 
        book_updated_on = :book_updated_on WHERE book_id = :book_id";

		$statement = $connect->prepare($query);

		$statement->execute($data);

		header('location:book_admin.php?msg=edit');
	}
}
...
if(isset($_GET["action"], $_GET["code"], $_GET["status"]) && $_GET["action"] == 'delete')
{
	...
	$query = "UPDATE lms_book SET book_status = :book_status, book_updated_on = :book_updated_on WHERE book_id = :book_id";

	$statement = $connect->prepare($query);

	$statement->execute($data);

	header('location:book_admin.php?msg='.strtolower($status).'');
}
...
<div class="container-fluid py-4" style="min-height: 700px;">
	...
	<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
        ...
        <li class="breadcrumb-item"><a href="book_admin.php">Book Management</a></li>
		...
		<div class="card-header">
			<div class="row">
                ...
                <div class="col col-md-6" align="right">
                	<a href="book_admin.php?action=add" class="btn btn-success btn-sm">Add</a>
                </div>
            </div>
        </div>
        <div class="card-body">
        		...
        		if($statement->rowCount() > 0)
        		{
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				<tr>
        					...
        					<td>
        						<a href="book_admin.php?action=edit&code='.convert_data($row["book_id"]).'" class="btn btn-sm btn-primary">Edit</a>
    ...
    <script>
    	...
    	function delete_data(code, status)
    	{
    		...
    		{
    			window.location.href = "book_admin.php?action=delete&code="+code+"&status="+new_status+"";
    		}
    	}
